...
let registerValidator = async (req) => {
	...
	req.check("username", "Username must be more than 2 characters").isLength({
		min: 3,
	});
	...
	req.check("password_confirmation", "password_confirmation is required.")
		.not()
		.isEmpty();
	req.check("password_confirmation", "Password mismatch").equals(
		req.body.password
	);
